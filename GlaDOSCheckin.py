...
BARK_PUSH = 'https://api.day.app/ra7fgu4whtBQKbqVxeb9Bj'  # bark自建服务器，要填完整链接，结尾的/不要
...
def get_cookie():
    ...
    for i in range(len(json.loads(response.text)['data'])):
        if json.loads(response.text)['data'][i]['name'] =='GlACK':
            try:
                env = json.loads(response.text)['data'][i]['value']
                cookies.append(env)
            except:
                pass
    return cookies
...
def get_info(cookie):
    ...
    try:
        if result['code'] == 0:
            ...
            msg.append("当前账号：" + result['data']['email'] +" , 剩余天数：" + str(float(result['data']['leftDays'])))
        else:
            print("cookie失效")
            msg.append("cookie失效")
    except:
...
def task(cookie):
    ...
    try:
        if result['code'] == 0:
            print(f"{result['message']} ，剩余天数：{float(result['list'][0]['balance'])}天")
            msg.append(result['message']+ " ，剩余天数："+str(float(result['list'][0]['balance'])))
        else:
            print(f"{result['message']},已签到明天再尝试")
            msg.append(result['message']+"已签到明天再尝试")
    except:
        print("未知错误，检查cookie格式")
    return msg
